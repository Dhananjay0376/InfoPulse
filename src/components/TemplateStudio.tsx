...
export default function TemplateStudio({ templates, loading, canManage, onCreate }: Props) {
      ...
      <div className="rounded-3xl bg-white p-6 shadow-xl shadow-slate-200/50 ring-1 ring-slate-100 sm:p-8">
        <div className="flex items-start justify-between gap-4">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-black text-slate-900">{canManage ? "Create an email template" : "Template preview"}</h2>
          </div>
          <div className="rounded-2xl bg-indigo-50 p-3 text-indigo-600">
            {canManage ? <FileText size={20} /> : <Lock size={20} />}
          </div>
        </div>
        ...
        {canManage ? (
          ...
          <div className="mt-6 rounded-2xl bg-slate-50 px-4 py-5 text-sm text-slate-600">
            Viewer accounts can inspect templates and campaign history but cannot create or modify sending assets.
          </div>
        )}
      </div>
